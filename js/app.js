(() => {
    ...
    function setSizeInfo() {
        ...
        document.querySelector(".window-size").innerHTML = getWindowWidth();
    }
    function watchResize() {
        if (getWindowWidth() < 1400 || getWindowWidth() > 1600) {
            document.querySelector("body").classList.add("_is-preloader");
    ...
    document.querySelector("button").addEventListener("click", (() => {
        if (getWindowWidth() < 1400) alert("Число меньше за 1400 -_-"); else if (getWindowWidth() > 1600) alert("Число больше за 1600 -_-"); else {
            document.querySelector("body").classList.remove("_is-preloader");
            window.removeEventListener("resize", setSizeInfo);
...
*/
    ...
    function getText(e) {
        ...
        if (type === 1 || type === 9 || type === 11) if (typeof e.textContent === "string") return e.textContent; else for (e = e.firstChild; e; e = e.nextSibling) result += getText(e); else if (type === 3 || type === 4) return e.nodeValue;
        return result;
    }
...
*/
    var TextPlugin_gsap, TextPlugin_tempDiv, TextPlugin_getGSAP = function _getGSAP() {
        return TextPlugin_gsap || typeof window !== "undefined" && (TextPlugin_gsap = window.gsap) && TextPlugin_gsap.registerPlugin && TextPlugin_gsap;
    };
    var TextPlugin = {
        version: "3.12.7",
        name: "text",
        init: function init(target, value, tween) {
            ...
            var _short, text, original, j, condensedText, condensedOriginal, aggregate, s, i = target.nodeName.toUpperCase(), data = this, _value = value, newClass = _value.newClass, oldClass = _value.oldClass, preserveSpaces = _value.preserveSpaces, rtl = _value.rtl, delimiter = data.delimiter = value.delimiter || "", fillChar = data.fillChar = value.fillChar || (value.padSpace ? "&nbsp;" : "");
            ...
            TextPlugin_tempDiv || (TextPlugin_tempDiv = document.createElement("div"));
            TextPlugin_tempDiv.innerHTML = value.value;
            text = splitInnerHTML(TextPlugin_tempDiv, delimiter, false, preserveSpaces, data.svg);
        ...
        render: function render(ratio, data) {
            ...
            var applyNew, applyOld, str, text = data.text, hasClass = data.hasClass, newClass = data.newClass, oldClass = data.oldClass, delimiter = data.delimiter, target = data.target, fillChar = data.fillChar, original = data.original, rtl = data.rtl, l = text.length, i = (rtl ? 1 - ratio : ratio) * l + .5 | 0;
            if (hasClass && ratio) {
                applyNew = newClass && i;
    ...
    setTimeout((() => {
        gsapWithCSS.utils.toArray(".block-wrapper").forEach(((blockWrapper, index) => {
            const block = blockWrapper.querySelector(".block");
            if (index === 0) {
                if (block) block.style.zIndex = "1";
                blockWrapper.style.zIndex = "1";
            }
            gsapWithCSS.to(blockWrapper, {
                scrollTrigger: {
                    ...
                    onLeave(e) {
                        if (block) block.style.removeProperty("z-index");
                        blockWrapper.style.removeProperty("z-index");
                    },
                    onLeaveBack(e) {
                        if (block) block.style.removeProperty("z-index");
                        blockWrapper.style.removeProperty("z-index");
        ...
        const mainTitleConfig = {
            trigger: document.querySelector(".main-title").closest(".block-wrapper"),
            start: "top top",
            end: "bottom top-=50%",
            scrub: true
        };
        gsapWithCSS.to(".main-title", {
            scrollTrigger: mainTitleConfig,
            opacity: 0,
            scale: 2.5
        });
        gsapWithCSS.to(".main-title--top", {
            scrollTrigger: mainTitleConfig,
            yPercent: -50,
            opacity: 0
        });
        gsapWithCSS.to(".main-title--bottom", {
            scrollTrigger: mainTitleConfig,
            yPercent: 50,
            opacity: 0
        });
        gsapWithCSS.utils.toArray(".text").forEach(((text, index) => {
            let tl = gsapWithCSS.timeline({
                scrollTrigger: {
                    trigger: text.closest(".block-wrapper"),
                    start: "top bottom",
                    end: "bottom top",
                    scrub: true
                }
            });
            if (text.classList.contains("text--up")) tl.fromTo(text, {
                scale: .5,
                opacity: 0
            }, {
                scale: 1.2,
            ...
            if (text.classList.contains("text--down")) tl.fromTo(text, {
                y: "400px",
                scale: .5,
                opacity: 0
            }, {
            ...
            if (text.classList.contains("text--down-slow")) tl.fromTo(text, {
                y: "600px",
                scale: .5,
                opacity: 0
            }, {
            ...
            if (text.classList.contains("text--raiden")) {
                tl.fromTo(text, {
                    opacity: 0,
                    zIndex: -1
                }, {
            ...
            if (text.classList.contains("text--last")) tl.fromTo(text, {
                scale: .5,
                opacity: 0
            }, {
                scale: 1.2,
            ...
            if (text.classList.contains("text--bef-gal")) gsapWithCSS.timeline({
                scrollTrigger: {
                    trigger: text.closest(".block-wrapper"),
                    start: "top bottom",
                    end: "bottom top",
        ...
        const compliments = {
            top: [ "утонченная", "особенная", "красивая" ],
            bottom: [ "женственная", "нежная", "милая" ]
        };
        document.querySelectorAll(".gallery-item__text--top").forEach(((item, i) => {
            splitWord(item, compliments.top[i]);
        }));
        document.querySelectorAll(".gallery-item__text--bottom").forEach(((item, i) => {
            splitWord(item, compliments.bottom[i]);
        }));
        function splitWord(wrapper, word) {
            if (!word) return;
        ...
        const galleryTextTop = document.querySelectorAll(".gallery-item__text--top");
        const galleryTextBottom = document.querySelectorAll(".gallery-item__text--bottom");
        [ ...galleryTextTop, ...galleryTextBottom ].forEach(((item, i) => {
            item.querySelectorAll("span").forEach(((char, j) => {
                ScrollTrigger_ScrollTrigger.create({
                    trigger: item.closest(".block-wrapper"),
                    start: "top 30%",
                    end: "bottom top",
                    once: true,
                    onEnter: () => {
                        gsapWithCSS.timeline().to(char, {
                            opacity: 1,
                            y: 0,
                            duration: .6,
                            ease: "bounce.out"
        ...
        splitWord(galleryText, "Квиточек");
        galleryText.querySelectorAll("span").forEach(((char, index) => {
            ScrollTrigger_ScrollTrigger.create({
                trigger: galleryText,
        ...
        gsapWithCSS.utils.toArray(".gallery--compliments .gallery__item").forEach(((item, index) => {
            let config;
            switch (index) {
              case 0:
                config = {
        ...
        gsapWithCSS.utils.toArray(".gallery--body .gallery__item").forEach(((item, index) => {
        ...
        ScrollTrigger_ScrollTrigger.create({
            trigger: document.querySelector(".raiden").closest(".block-wrapper"),
            start: "top center",
            end: "bottom center+=40%",
            onEnter: () => {
                document.querySelector(".raiden").classList.add("show");
            },
            onEnterBack: () => {
                document.querySelector(".raiden").classList.add("show");
            },
            onLeave: () => {
                document.querySelector(".raiden").classList.remove("show");
            },
            onLeaveBack: () => {
                document.querySelector(".raiden").classList.remove("show");
            }
    ...
    const video = document.querySelector(".gallery-item__video");
    const videoElem = video.querySelector("video");
    const playBtn = video.querySelector(".gallery-item__video-play");
